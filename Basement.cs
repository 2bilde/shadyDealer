class basement
{
    ...
    public basement(Customer cus) //
    {
        ...
        if (attempt == secretCode){
            ...
            Console.WriteLine("As you can see we have a lot in stock right now so what would you like? Keep in mind we dont have much time so you can only chose 1.");
            ...
            for (int i = 0; i < drugList.Count; i++){
                Console.WriteLine("[" + (i+1) + "] - " + drugList[i].Item1);
            }
            int choice;
            ...
            if (drugList[choice-1].Item2 * 100 > cus.budget){
                ...
                {
                Console.WriteLine("I'm sorry? i dont think i heard that right. This aint no small business buddy. we operating in MINIMUM 100grams. So how much did you want again?");
